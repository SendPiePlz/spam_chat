...
void main() {
  ...
  test('URL parsing tests', () {
    final t = UrlTransformer();
    const url = 'www.gailly.net/go.html';
    final cat = url.split(RegExp(r'\W|[\-_]')).map(t.categorize).toList();

    expect(cat, [6,5,6,6,2]);
    expect(t.getPathStart(url), 14);

    expect(t.transform(url), [0,3,1,0,0,0,0,0,14,8,1,0,0,0,3,1]);
  });
}